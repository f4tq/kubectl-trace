...
var (
	...
	ImageNameTag = "docker.io/f4tq/kubectl-ethos:latest"
...
var (
	runShort = `Execute a bcc program on resources` // Wrap with i18n.T()
  ...
  %[1]s ethos  run node/kubernetes-node-emt8.c.myproject.internal -f tcpconnect.py 
`
...
func NewRunCommand(factory factory.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().StringVarP(&o.program, "filename", "f", o.program, "File containing a bpftrace, python, or self managed")
	...
	cmd.Flags().BoolVarP(&o.cannedCmd, "provided-bin", "", o.cannedCmd, "Use a command from the the tracerunner's image")
...
func (o *RunOptions) Run() error {
	...
	tj := Job{
		Name:                fmt.Sprintf("%s%s", meta.EthosObjectNamePrefix, string(juid)),
